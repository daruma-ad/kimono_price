...
export default function Home() {
  ...
  const conditionFactor = condition === "未使用に近い" ? 1.15 : condition === "美品" ? 1 : condition === "良好" ? 0.82 : condition === "シミ・使用感あり" ? 0.58 : 0.7;
  const evidenceFactor = (certificate ? 1.12 : 1) * (selvedge ? 1.04 : 1) * (signature ? 1.06 : 1);
  const makerFactor = maker.trim() && maker !== unknownValue ? 1.05 : 1;
      ...
      {active === "assess" ? <div className="assessment">
        <section className="panel input-panel"><div className="section-label">STEP 1 / 写真</div><h2>着物の写真を追加</h2><p className="muted">全体・衿元・証紙・気になる箇所があると、より参考になります。</p>
          ...
          <div className="form-grid">
            ...
            <label>産地・工芸名<select value={origin} onChange={(e) => setOrigin(e.target.value)}>{Object.entries(originGroups).map(([group, origins]) => <optgroup label={group} key={group}>{origins.map((item) => <option key={item}>{item}</option>)}</optgroup>)}<option>{unknownValue}</option></select></label>
            <label>技法・特徴<select value={technique} onChange={(e) => setTechnique(e.target.value)}><option>泥染め</option><option>白大島</option><option>絣</option><option>手紡ぎ・絣</option><option>手描友禅</option><option>金彩</option><option>絞り</option><option>びんがた</option><option>花織</option><option>金糸</option><option>{unknownValue}</option></select></label>
            <label>織元・作家・ブランド<div className="maker-input"><input value={maker} onChange={(e) => setMaker(e.target.value)} placeholder="例：落款・織元名" /><button type="button" onClick={() => setMaker(unknownValue)}>不明</button></div></label>
            <label>格<select value={formality} onChange={(e) => setFormality(e.target.value)}><option>第一礼装</option><option>準礼装</option><option>略礼装</option><option>普段着</option><option>{unknownValue}</option></select></label>
            <label>季節・仕立て<select value={season} onChange={(e) => setSeason(e.target.value)}><option>袷</option><option>単衣</option><option>夏物</option><option>通年</option><option>未仕立て</option><option>{unknownValue}</option></select></label>
            <label>対象<select value={target} onChange={(e) => setTarget(e.target.value)}><option>女性</option><option>男性</option><option>子ども</option><option>兼用</option><option>{unknownValue}</option></select></label>
            <label>状態<select value={condition} onChange={(e) => setCondition(e.target.value)}><option>未使用に近い</option><option>美品</option><option>良好</option><option>シミ・使用感あり</option><option>{unknownValue}</option></select></label>
          </div>
          <div className="certificate-row"><span>証紙</span><label className="switch"><input type="checkbox" checked={certificate} onChange={(e) => setCertificate(e.target.checked)} /><span /></label><small>{certificate ? "あり" : "なし・不明"}</small><label className="evidence-check"><input type="checkbox" checked={selvedge} onChange={(e) => setSelvedge(e.target.checked)} />反端あり</label><label className="evidence-check"><input type="checkbox" checked={signature} onChange={(e) => setSignature(e.target.checked)} />落款あり</label><small className="evidence-note">未選択は不明・なし</small></div>
